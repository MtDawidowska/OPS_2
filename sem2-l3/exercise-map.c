...
void child_work(int n, float* out, char* log)
{
    ...
    snprintf(buf, LOG_LEN + 1, "%7.5f\n", out[n] * 4.0f); // great for formating strings
    memcpy(log + n * LOG_LEN, buf, LOG_LEN); // copying chunks of memory
}
...
void parent_work(int n, float* data)
{
    pid_t pid;
    double sum = 0.0;
    while( !((pid=wait(NULL)) <= 0 && errno == ECHILD)) ; // wait for all children to terminate
